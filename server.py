...
class ThroughputServerProtocol(QuicConnectionProtocol):
            ...
            if event.data.startswith(b"init"):
                ...
                if len(parts) > 1:
                    try:
                        ...
                        logger.error("Server: Error parsing duration, defaulting to 10s: %s", e)
                        duration = 10
                else:
                    duration = 10
                logger.info("Server: Received trigger on stream %d, starting %d-second transmission.", event.stream_id, duration)
    ...
    async def handle_stream(self, stream_id: int, duration: int):
        ...
        mb_s = (bytes_sent / elapsed) / (1024 * 1024)
        logger.info(
            "Server: Finished sending %d bytes in %.2f s (%.2f MB/s) on stream %d",
            bytes_sent, elapsed, mb_s, stream_id
        )
...
async def main():
    ...
    await serve("127.0.0.1", 5001, configuration=configuration, create_protocol=ThroughputServerProtocol)
